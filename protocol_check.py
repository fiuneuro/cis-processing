...
def get_parser():
    parser = argparse.ArgumentParser(description='Check scans for project protocol compliance.')
...
def main(argv=None):
    ...
    with open(op.join(os.path.dirname(args.bids_dir), 'code/config.json'), 'r') as fo:
        config_options = json.load(fo)
        
    if not op.isfile(op.join(os.path.dirname(args.bids_dir), config_options['protocol'])):
    ...
    if not os.path.isdir(op.join(args.work_dir, args.sub, args.ses)):
        raise ValueError('Session directory does not exist in subjects working directory.')

    with open(op.join(os.path.dirname(args.bids_dir), config_options['protocol']), 'r') as fo:
    ...
    for tmp_scan in scans:
        if (tmp_scan != 'email') and (tmp_scan != 'project'):
            ...
            if len(tmp_scan_list) != num:
                warning = True
                with open(op.join(args.work_dir, '{sub}-{ses}-protocol_error.txt'.format(sub=args.sub, ses=args.ses)), 'a') as fo:
                    fo.write('There are {0} scans for {1}, but should be {2} \n'.format(len(tmp_scan_list), tmp_scan, num))
                  
            for t in tmp_scan_list:
                if len(os.listdir(op.join(args.work_dir, args.sub, args.ses, t, 'resources/DICOM/files'))) != dicoms:
                    warning = True
                    with open(op.join(args.work_dir, '{sub}-{ses}-protocol_error.txt'.format(sub=args.sub, ses=args.ses)), 'a') as fo:
                        fo.write('There are {0} DICOMs for {1}, but should be {2} \n'.format(len(os.listdir(op.join(args.work_dir, args.sub, args.ses, t, 'resources/DICOM/files'))), t, dicoms))
