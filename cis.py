...
def main(bids_dir, config, work_dir=None, protocol_check=False, autocheck=False,
    ...
    if autocheck:
        ...
        cmd = ('{sing} -w {work} --project {proj} --session {xnat_exp} --processed '
               '{tar_list}'.format(
                    sing=scratch_xnatdownload,
                    work=proj_work_dir,
                    proj=config_options['project'],
                ...
                for tmp_ses in ses_list:
                    ...
                    if protocol_check:
                        cmd = ('python {fdir}/protocol_check.py -w {work} '
                               '--bids_dir {bids_dir} --sub {sub} --ses {ses}'.format(
                                    fdir=fdir,
                                    work=raw_work_dir,
                                    bids_dir=bids_dir,
                                    sub=tmp_sub,
                    ...
                    tar_file = op.join(
                        raw_dir,
                        '{sub}/{ses}/{sub}-{ses}.tar'.format(sub=tmp_sub, ses=tmp_ses)
                    )
                    with tarfile.open(tar_file, 'w') as tar:
                        tar.add(op.join(raw_work_dir, tmp_sub),
