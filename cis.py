...
def main(argv=None):
    ...
    scratch_xnatdownload = op.join(args.work_dir,
                                   op.basename(xnatdownload_file))
    ...
    if args.autocheck:
        ...
        raise Exception('A valid XNAT Experiment \
            session was not entered for the project \
            or you are not running autocheck.')
    ...
    if op.isdir(raw_work_dir):
        ...
        if data_download:
            ...
            for tmp_sub in sub_list:
                ses_list = os.listdir(op.join(raw_work_dir,
                                              '{0}'.format(tmp_sub)))
                ...
                for tmp_ses in ses_list:
                    ...
                    with tarfile.open(op.join(raw_dir,
                                              ...
                                              '{sub}-{ses}.tar'.format(sub=tmp_sub,
                                                                       ses=tmp_ses)), "w") as tar:
                        tar.add(op.join(raw_work_dir,
                                        '{sub}'.format(sub=tmp_sub)),
                                arcname=os.path.basename(op.join(raw_work_dir,
                                                                 '{sub}'.format(sub=tmp_sub))))
                        shutil.rmtree(op.join(raw_work_dir,
                                              '{sub}'.format(sub=tmp_sub)))

                    scans_df = pd.read_csv(op.join(raw_dir,
                                                   'scans.tsv'), sep='\t')
                    tmp_df = pd.DataFrame()
                    tmp_df = tmp_df.append({'sub': tmp_sub}, ignore_index=True)
                    tmp_df['ses'] = tmp_ses
                    tmp_df['file'] = '{sub}-{ses}.tar'.format(sub=tmp_sub,
                                                              ses=tmp_ses)
                    moddate = os.path.getmtime(op.join(raw_dir,
                                                       tmp_sub,
                                                       tmp_ses,
                                                       '{sub}-{ses}.tar'.format(sub=tmp_sub,
                                                                                ses=tmp_ses)))
                    timedateobj = datetime.datetime.fromtimestamp(moddate)
                    tmp_df['creation'] = datetime.datetime.strftime(timedateobj,
                                                                    "%m/%d/%Y, %H:%M")
                    scans_df = scans_df.append(tmp_df)
                    scans_df.to_csv(op.join(raw_dir,
                                            'scans.tsv'),
                                    sep='\t',
                                    index=False)
                    ...
                    with open(op.join(proj_work_dir,
                        ...
                        fo.write('Data transferred from XNAT to \
                                  FIU-HPC for Project: {proj} \
                                  Subject: {sub} \
                                  Session: {ses} on \
            ...
            cmd = ("mail -s 'FIU XNAT-HPC Data Transfer Update Project \
                    {proj}' {email_list} < {message}".format(proj=config_options['project'],
